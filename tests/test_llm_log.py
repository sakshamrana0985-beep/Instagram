...
def test_estimate_cost_uses_per_model_pricing():
    cost = estimate_cost_usd("gemini-embedding-001", input_tokens=1_000_000, output_tokens=0)
    assert cost == pytest.approx(0.15)


def test_unpriced_model_reports_unknown_rather_than_a_made_up_number():
...
def test_log_call_records_model_tokens_latency_and_cost(caplog):
    with caplog.at_level(logging.INFO, logger="recall.llm"):
        with log_call("gemini-embedding-001", "summarize", content_type="listicle") as slot:
            slot.append(_response(15_000, 400))

    record = caplog.records[-1].getMessage()
    assert "model=gemini-embedding-001" in record
    assert "stage=summarize" in record
    assert "input_tokens=15000" in record
    assert "output_tokens=400" in record
    assert "est_cost_usd=0.002250" in record
    assert "latency=" in record
    assert "content_type=listicle" in record


def test_log_call_reports_unknown_cost_for_an_unpriced_model(caplog):
    with caplog.at_level(logging.INFO, logger="recall.llm"):
        with log_call("gemini-3.5-flash", "summarize") as slot:
            slot.append(_response(15_000, 400))

    assert "est_cost_usd=unknown" in caplog.records[-1].getMessage()
...
def test_log_call_logs_and_reraises_failures(caplog):
    with caplog.at_level(logging.INFO, logger="recall.llm"):
        with pytest.raises(RuntimeError, match="model down"):
            with log_call("gemini-3.5-flash", "summarize"):
